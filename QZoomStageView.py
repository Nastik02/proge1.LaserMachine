...
class QZoomStageView(QWidget):
    ...
    def showEvent(self, e) -> None:
        self.__cameraPosition = QPointF(10, 40)
        self.set_zoom(10)
    ...
    def setCurrentState(self, state: MarkerState):
        self.__markerState = state
        self.update()
    ...
    def paintEvent(self, e: QPaintEvent):
        ...
        painter.fillRect(e.rect(), QColor(0,50,100))
        painter.save()
        matrix = QTransform()
        matrix.scale(self.__zoom, self.__zoom)  # инвертируем для привычной сетки осей
        matrix.translate(self.__cameraPosition.x(), self.__cameraPosition.y())  # - т.к зумили в -1 по y
        ...
        painter.setPen(QPen(QColor(0,5,168), 2.0 / self.__zoom))
        inverted, result = matrix.inverted()
